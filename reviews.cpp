...
void readKeywordWeights(istream &input, vector<string> &keywords, vector<double> &weights) {
    ...
    string word;
    double num;
    //reads in the string and number and stores it into the vector
...
void readReview(istream &input, vector<string> &reviewWords) {
    // TODO: Write an implementation for this function!
    string word;
    //reads in each word and stores it into the vector
    while(input >> word){
...
double wordWeight(const string &word, const vector<string> &keywords, const vector<double> &weights) {
    ...
    for(int i = 0; i < keywords.size(); ++i){
        //if the word is found as a keyword, then return the weight 
        if(word == keywords.at(i)){
            return weights.at(i);
        }
    }
    //if it wasn't found then returns 0 as the weight
    return 0;
}
...
double reviewScore(const vector<string> &reviewWords, const vector<string> &keywords, const vector<double> &weights) {
    // TODO: Write an implementation for this function!
    vector<string> copyWords = reviewWords;
    preprocessReview(copyWords);
    double totalScore;
    for(int i = 0; i < copyWords.size(); ++i){
        //loops through the whole review and looks for the weights and adds them up
        totalScore += wordWeight(copyWords.at(i), keywords, weights);
    }
    return totalScore;
}
